...
$regPath = "HKLM:\Software\Poseidon"
...
if ([string]::IsNullOrWhiteSpace($installDir)) {
    $installDir = Join-Path $env:ProgramFiles "Poseidon"
}

$summary.InstallDirExists = Test-Path -LiteralPath $installDir
$desktopExePath = Join-Path $installDir "Poseidon.Desktop.exe"
...
$shortcuts = foreach ($location in $shortcutScanLocations) {
    ...
    Get-ChildItem -LiteralPath $location -Filter *.lnk -File -Recurse -ErrorAction SilentlyContinue |
        ForEach-Object {
            ...
            if ($link.TargetPath -notmatch "Poseidon\.Desktop\.exe$") {
                return
            }
...
$desktopProcesses = Get-CimInstance -ClassName Win32_Process -Filter "Name='Poseidon.Desktop.exe'" -ErrorAction SilentlyContinue
...
$service = Get-Service -Name "Poseidon WorkerService" -ErrorAction SilentlyContinue
if ($null -ne $service) {
    ...
    [pscustomobject]@{
        ...
        StartType = (Get-CimInstance -ClassName Win32_Service -Filter "Name='Poseidon WorkerService'" -ErrorAction SilentlyContinue).StartMode
    } | Export-Object -Path (Join-Path $outputDir "worker-service.json")
}
else {
    [pscustomobject]@{
        Name = "Poseidon WorkerService"
        Status = "NotInstalled"
    } | Export-Object -Path (Join-Path $outputDir "worker-service.json")
}

$desktopLogDir = Join-Path $env:LOCALAPPDATA "Poseidon\Logs"
$summary.DesktopLogsFolderExists = Test-Path -LiteralPath $desktopLogDir
if ($summary.DesktopLogsFolderExists) {
    $desktopLogs = Get-ChildItem -LiteralPath $desktopLogDir -Filter "poseidon-*.log" -File -ErrorAction SilentlyContinue
    $desktopErrorLogs = Get-ChildItem -LiteralPath $desktopLogDir -Filter "poseidon-errors-*.log" -File -ErrorAction SilentlyContinue
    ...
    Copy-LatestLogs -SourceDir $desktopLogDir -Filter "poseidon-*.log" -DestinationDir (Join-Path $outputDir "logs") | Out-Null
    Copy-LatestLogs -SourceDir $desktopLogDir -Filter "poseidon-errors-*.log" -DestinationDir (Join-Path $outputDir "logs") | Out-Null
}

$workerInstanceRoot = Join-Path $env:LOCALAPPDATA "Poseidon\Instances"
...
$appEvents = Get-WinEvent -FilterHashtable @{ LogName = "Application"; StartTime = $startTime } -ErrorAction SilentlyContinue |
    Where-Object {
        ...
        $_.Message -match "Poseidon|Poseidon\.Desktop"
    }
