...
public sealed class ModelIntegrityServiceTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"Poseidon_ModelTests_{Guid.NewGuid():N}");
    ...
    [Fact]
    public async Task VerifyOnStartup_LlmModelExists_NoHash_ProductionInvalid()
    {
        CreateModelFile("qwen2.5-14b.Q5_K_M.gguf");
        var svc = CreateService(new Dictionary<string, string?>
        {
            ["DOTNET_ENVIRONMENT"] = "Production",
            ["Security:AllowInsecureDevelopmentSecrets"] = "false"
        });
        await svc.VerifyOnStartupAsync();

        svc.LlmModelExists.Should().BeTrue();
        svc.LlmModelValid.Should().BeFalse();
        svc.LlmError.Should().Contain("ExpectedLlmHash");
    }
    ...
    [Fact]
    public async Task VerifyOnStartup_EmbeddingExists_NoHash_Onnx_ProductionInvalid()
    {
        CreateModelFile("qwen2.5-14b.Q5_K_M.gguf");
        CreateModelFile("arabert.onnx");
        var svc = CreateService(new Dictionary<string, string?>
        ...
        svc.EmbeddingModelExists.Should().BeTrue();
        svc.EmbeddingModelValid.Should().BeFalse();
        svc.EmbeddingError.Should().Contain("ExpectedEmbeddingHash");
    }
