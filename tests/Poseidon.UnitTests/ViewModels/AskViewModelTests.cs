...
public sealed class AskViewModelTests : IDisposable
{
    ...
    public async Task AskQuestion_LibraryOnlyMode_BlocksWithSafetyWarning()
    {
        ...
        vm.AnswerText.Should().Contain("recovery mode");
    ...
    public async Task AskQuestion_PopulatesCitations()
    {
        ...
        vm.Citations[0].FileName.Should().Be("?????_????????.pdf");
        vm.Citations[0].PageNumber.Should().Be(42);
        vm.Citations[0].Content.Should().Contain("?????? 42");
        vm.Citations[0].SimilarityScore.Should().BeApproximately(0.95, 0.01);

        vm.Citations[1].FileName.Should().Be("?????_?????????.pdf");
    ...
    public async Task AskQuestion_PopulatesWarnings()
    {
        ...
        vm.Warnings[0].Should().Be("????? 1");
        vm.Warnings[1].Should().Be("????? 2");
    }
    ...
    public async Task SafetyValidation_NoCitations_CriticalWarning()
    {
        ...
        vm.SafetyWarningText.Should().Contain("Critical safety warning");
    ...
    public async Task SafetyValidation_ModeratelyLowConfidence_WarningLevel()
    {
        ...
        vm.SafetyWarningText.Should().Contain("Safety warnings present");
    }

    [Fact]
    public async Task SafetyValidation_FabricationIndicator_CriticalWarning()
    {
        // Answer contains an external-knowledge marker, so fabrication is detected.
        var answer = CreateHighConfidenceAnswer(
            text: "بناء على معرفتي القانونية العامة، يمكن القول...");
        SetupMediatorReturn(answer);
    ...
    {
        // Answer mentions article 99 but citation snippets don't contain 99.
        var answer = new LegalAnswer
        {
            Answer = "تنص المادة 99 على العقوبة...",
    ...
    {
        SetupMediatorReturn(CreateHighConfidenceAnswer(text: "????? ?????"));

        var vm = await CreateVmAsync();
        vm.Question = "????";
        await vm.AskQuestionCommand.ExecuteAsync(null);

        vm.QueryHistory[0].AnswerPreview.Should().Be("????? ?????");
    }

    [Fact]
    public async Task History_MultipleQueries_InsertedInReverseChronological()
    {
        var answer1 = CreateHighConfidenceAnswer(text: "????? ????");
        var answer2 = CreateHighConfidenceAnswer(text: "????? ?????");
        ...
        vm.QueryHistory[0].Question.Should().Be("???? 2", "most recent at top");
        vm.QueryHistory[1].Question.Should().Be("???? 1");
    }
    ...
    public async Task LoadFromHistory_Null_DoesNothing()
    {
        ...
        vm.Question.Should().Be("???? ????", "null item should not change question");
    }
