...
public sealed class EmbeddedVectorStoreTests : IDisposable
{
    ...
    public async Task UpsertAsync_PreservesAllChunkFields()
    {
        ...
        var chunk = new DocumentChunk
        {
            Id = "full",
            DocumentId = "doc_full",
            Content = "نص قانوني عربي للاختبار",
            ChunkIndex = 5,
            PageNumber = 3,
            SectionTitle = "الباب الأول",
            ArticleReference = "المادة 15",
            CaseNumber = "2024/100",
        ...
        c.Content.Should().Be("نص قانوني عربي للاختبار");
        c.ChunkIndex.Should().Be(5);
        c.PageNumber.Should().Be(3);
        c.SectionTitle.Should().Be("الباب الأول");
        c.ArticleReference.Should().Be("المادة 15");
        c.CaseNumber.Should().Be("2024/100");
        c.CourtName.Should().Be("محكمة النقض");
        ...
        c.SourceFileName.Should().Be("حكم_محكمة.pdf");
    }
    ...
    public async Task SearchAsync_ScoreThreshold_FiltersLowScores()
    {
        ...
        await sut.UpsertAsync([
            ...
            MakeChunk("ortho", "doc1", UnitVector(3)) // orthogonal → cosine ≈ 0
        ]);
    ...
    public async Task HnswRecall_ManyVectors_FindsClosestNeighbor()
    {
        ...
        // The exact nearest neighbor to UnitVector(0) — find it via brute force
