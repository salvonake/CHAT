...
/// <summary>
/// Tests for LegalRetrievalPipeline — the multi-stage retrieval pipeline.
...
public sealed class LegalRetrievalPipelineTests : IDisposable
{
    ...
    private static DocumentChunk MakeChunk(
        string id = "chunk1",
        string content = "نص قانوني للاختبار",
    ...
    public async Task RetrieveAsync_CallsEmbedder_ForEachVariant()
    {
        ...
        await pipeline.RetrieveAsync("اشرح القانون", DefaultConfig(hybrid: false), null, CancellationToken.None);
    ...
    // ══════════════════════════════════════
    //  Context Budget (Stage 4 — BuildContext)
    // ══════════════════════════════════════
    ...
    {
        SetupEmbedding();
        var chunk = MakeRetrievedChunk(sourceFile: "contract.pdf", page: 5, article: "المادة 12", caseNum: "Q-789", court: "محكمة النقض");
        SetupVectorSearch(chunk);
        ...
        result.AssembledContext.Should().Contain("المصدر");
        result.AssembledContext.Should().Contain("المادة 12");
        result.AssembledContext.Should().Contain("Q-789");
        result.AssembledContext.Should().Contain("محكمة النقض");
    }
    ...
    public async Task RetrieveAsync_ChunksExceedTokenBudget_Truncated()
    {
        ...
        var chunks = new[]
        {
            ...
            MakeRetrievedChunk("c2", 0.8f, hash: "h2", content: "نص ثاني", tokens: 2000),
            MakeRetrievedChunk("c3", 0.7f, hash: "h3", content: "نص ثالث", tokens: 2000),
        };
        SetupVectorSearch(chunks);
        ...
        // Only first 2 chunks fit the 4096 budget (2000 + 2000 = 4000 ≤ 4096)
    ...
    {
        SetupEmbedding();
        var chunk = MakeRetrievedChunk(content: "هذا نص مهم جداً للقانون");
        SetupVectorSearch(chunk);

        var pipeline = CreatePipeline();
        var result = await pipeline.RetrieveAsync("query", DefaultConfig(hybrid: false), null, CancellationToken.None);

        result.AssembledContext.Should().Contain("هذا نص مهم جداً للقانون");
    }
    ...
    public async Task RetrieveAsync_ContextIncludesChunkNumbering()
    {
        ...
        result.AssembledContext.Should().Contain("المصدر 1");
        result.AssembledContext.Should().Contain("المصدر 2");
    }
}
