...
$originalEnv = @{
    LEGALAI_RUN_INTEGRATION = [Environment]::GetEnvironmentVariable("LEGALAI_RUN_INTEGRATION", "Process")
    LEGALAI_QDRANT_HOST = [Environment]::GetEnvironmentVariable("LEGALAI_QDRANT_HOST", "Process")
    LEGALAI_QDRANT_PORT = [Environment]::GetEnvironmentVariable("LEGALAI_QDRANT_PORT", "Process")
    LEGALAI_ONNX_MODEL_PATH = [Environment]::GetEnvironmentVariable("LEGALAI_ONNX_MODEL_PATH", "Process")
}
...
try {
    ...
    $env:LEGALAI_QDRANT_HOST = $QdrantHost
    $env:LEGALAI_QDRANT_PORT = "$QdrantPort"
    ...
    if (-not [string]::IsNullOrWhiteSpace($OnnxModelPath)) {
        ...
        $env:LEGALAI_ONNX_MODEL_PATH = $OnnxModelPath
    }

    $configuration = if ($Release) { "Release" } else { "Debug" }
    $integrationAssemblyPath = "tests/LegalAI.IntegrationTests/bin/$configuration/net8.0/LegalAI.IntegrationTests.dll"
    ...
    if (-not $NoBuild) {
        dotnet build tests/LegalAI.IntegrationTests/LegalAI.IntegrationTests.csproj --configuration $configuration
