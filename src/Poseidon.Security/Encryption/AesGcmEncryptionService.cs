...
namespace Poseidon.Security.Encryption;
...
///     â†’ Vector Encryption Key (HKDF, info="vector-encryption")
///     â†’ Audit Signing Key    (HKDF, info="audit-signing")
///     â†’ Session HMAC Key     (HKDF, info="session-hmac")
/// </summary>
public sealed class AesGcmEncryptionService : IEncryptionService
{
    ...
    private static byte[] DeterministicSalt(string passphrase)
    {
        return SHA256.HashData(
            Encoding.UTF8.GetBytes($"Poseidon-Salt-{passphrase.Length}"));
    }
}
