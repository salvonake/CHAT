...
public sealed class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> BootstrapAdmin([FromBody] BootstrapAdminRequest request, CancellationToken ct)
    {
        ...
        {
            return BadRequest(new { error = _text.T("UsernamePasswordRequired", language) });
        }

        var count = await _users.GetCountAsync(ct);
        if (count > 0)
        {
            return Conflict(new { error = _text.T("BootstrapRequiresNoUsers", language) });
        }
        ...
        return Ok(new { message = _text.T("AdminUserCreated", language) });
    }
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request, CancellationToken ct)
    {
        ...
        if (user is null || user.IsDisabled)
        {
            ...
            return Unauthorized(new { error = _text.T("InvalidCredentials", language) });
        }
    ...
    public async Task<IActionResult> CreateUser([FromBody] CreateUserRequest request, CancellationToken ct)
    {
        ...
        {
            return BadRequest(new { error = _text.T("InvalidRole", language) });
        }
        ...
        {
            return Conflict(new { error = _text.T("UsernameAlreadyExists", language) });
        }
    ...
    public async Task<IActionResult> DisableUser(string id, [FromBody] DisableUserRequest request, CancellationToken ct)
    {
        ...
        if (existing is null)
        {
            return NotFound(new { error = _text.T("UserNotFound", language) });
        }
        ...
        return Ok(new { message = _text.T("UserStatusUpdated", language) });
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            return BadRequest(new { error = _text.T("DomainIdRequired", language) });
        }
        ...
        CancellationToken ct)
    {
        ...
        return Ok(new { message = _text.T("DomainAccessRevoked", language) });
    }
