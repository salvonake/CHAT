...
using DomainQueryAnalysis = Poseidon.Domain.ValueObjects.QueryAnalysis;

namespace Poseidon.Retrieval.QueryAnalysis;
...
public sealed partial class LegalQueryAnalyzer : IDomainQueryAnalyzer
{
    ...
    [GeneratedRegex(@"(?:ما هي|ماذا|ما هو|عرف|تعريف)", RegexOptions.Compiled)]
    private static partial Regex FactLookupPattern();

    [GeneratedRegex(@"(?:اشرح|وضح|فسر|كيف|لماذا)", RegexOptions.Compiled)]
    private static partial Regex ExplanationPattern();

    [GeneratedRegex(@"(?:قارن|الفرق|مقارن[ةه]|بين\s+.+\s+و)", RegexOptions.Compiled)]
    private static partial Regex ComparisonPattern();

    [GeneratedRegex(@"(?:تسلسل|ترتيب|متى|متي|تاريخ|زمني)", RegexOptions.Compiled)]
    private static partial Regex TimelinePattern();

    [GeneratedRegex(@"(?:سوابق|سابق[ةه]|قضي[ةه] مشابه[ةه]|حكم مماثل)", RegexOptions.Compiled)]
    private static partial Regex PrecedentPattern();

    [GeneratedRegex(@"(?:الماد[ةه]|ماد[ةه]|الفصل|فصل|البند)\s*(?:رقم\s*)?([\d\u0660-\u0669]+)", RegexOptions.Compiled)]
    private static partial Regex ArticleDetection();

    [GeneratedRegex(@"(?:القضي[ةه]|قضي[ةه]|الدعوى|الدعوي|دعوى|دعوي)\s*(?:رقم\s*)?([\d\u0660-\u0669/\-]+)", RegexOptions.Compiled)]
    private static partial Regex CaseDetection();

    [GeneratedRegex(@"(?:محكم[ةه]|المحكم[ةه])\s+([\u0600-\u06FF\s]+?)(?:\s|$)", RegexOptions.Compiled)]
    ...
    private static List<string> GenerateVariants(string query, QueryType type)
    {
        ...
        switch (type)
        {
            case QueryType.PrecedentSearch:
                variants.Add(query.Replace("سوابق", "احكام سابقه"));
                variants.Add(query.Replace("سابقه", "حكم قضائي مماثل"));
                break;

            case QueryType.ArticleSearch:
                // Add variant with expanded article reference
                variants.Add(query.Replace("الماده", "نص الماده القانونيه"));
                variants.Add(query + " تطبيق تفسير");
                break;

            case QueryType.Comparison:
                variants.Add(query + " اوجه التشابه والاختلاف");
                break;
