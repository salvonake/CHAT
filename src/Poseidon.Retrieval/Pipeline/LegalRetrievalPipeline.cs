...
namespace Poseidon.Retrieval.Pipeline;
...
public sealed class LegalRetrievalPipeline : IRetrievalPipeline
{
        ...
        string? datasetScope)
    {
        ...
        // Stage 3: Reranking (optional â€” placeholder for cross-encoder)
        ...
        List<RetrievedChunk> chunks, int maxTokens)
    {
        ...
        foreach (var chunk in chunks)
        {
            ...
            sb.AppendLine($"--- [Ø§Ù„Ù…ØµØ¯Ø± {chunkNum}: {chunk.Chunk.SourceFileName}ØŒ ØµÙØ­Ø© {chunk.Chunk.PageNumber}] ---");

            if (chunk.Chunk.ArticleReference is not null)
                sb.AppendLine($"[Ù…Ø±Ø¬Ø¹ Ø§Ù„Ù…Ø§Ø¯Ø©: {chunk.Chunk.ArticleReference}]");

            if (chunk.Chunk.CaseNumber is not null)
                sb.AppendLine($"[Ø±Ù‚Ù… Ø§Ù„Ù‚Ø¶ÙŠØ©: {chunk.Chunk.CaseNumber}]");

            if (chunk.Chunk.CourtName is not null)
                sb.AppendLine($"[Ø§Ù„Ù…Ø­ÙƒÙ…Ø©: {chunk.Chunk.CourtName}]");
