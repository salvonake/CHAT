...
public partial class HealthViewModel : ObservableObject
{
    ...
    private async Task CheckDocumentsAsync()
    {
        try
        {
            ...
            IndexedDocuments = docs.Count(d => d.Status == LegalAI.Domain.Entities.DocumentStatus.Indexed);
            PendingDocuments = docs.Count(d =>
                d.Status == LegalAI.Domain.Entities.DocumentStatus.Pending ||
                d.Status == LegalAI.Domain.Entities.DocumentStatus.Indexing);
            QuarantinedDocuments = docs.Count(d => d.Status == LegalAI.Domain.Entities.DocumentStatus.Quarantined);
        }
        catch (Exception ex)
        {
