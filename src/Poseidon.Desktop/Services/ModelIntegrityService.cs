...
public class ModelIntegrityService
{
    ...
    private readonly SecurityValidationContext _securityContext;
        ...
        ILogger<ModelIntegrityService> logger)
    {
        ...
        _paths = paths;
        _logger = logger;
        _securityContext = SecurityValidationContext.FromConfiguration(config);
    }
    ...
    {
        var provider = _config["Llm:Provider"]?.ToLowerInvariant() ?? "llamasharp";
        if (provider == "ollama")
        {
            LlmModelValid = true;
            LlmModelExists = true;
        ...
        var modelPath = ModelPathResolver.ResolveLlmPath(_config, _paths);
        ...
        {
            LlmModelValid = !_securityContext.IsProductionLike;
            LlmError = _securityContext.IsProductionLike
                ? "ModelIntegrity:ExpectedLlmHash is required in production."
                : null;
            _logger.LogWarning(
                "LLM model found at {Path} but no integrity hash is configured. Production mode treats this as invalid.",
                modelPath);
            return;
        }
    ...
    private void VerifyEmbeddingModel()
    {
        ...
        var modelPath = ModelPathResolver.ResolveEmbeddingPath(_config, _paths);
        ...
        if (string.IsNullOrEmpty(expectedHash))
        {
            EmbeddingModelValid = !_securityContext.IsProductionLike;
            EmbeddingError = _securityContext.IsProductionLike
                ? "ModelIntegrity:ExpectedEmbeddingHash is required in production."
                : null;
            _logger.LogWarning(
                "Embedding model found at {Path} but no integrity hash is configured. Production mode treats this as invalid.",
                modelPath);
            return;
        }
