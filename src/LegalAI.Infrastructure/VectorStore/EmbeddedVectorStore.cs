...
/// graph for approximate nearest-neighbor search. Zero external dependencies — no Docker,
/// no Qdrant. Designed for single-machine enterprise deployment.
/// 
/// Storage layout:
///   vectors.db   — SQLite database with chunk metadata + raw embedding blobs
///   vectors.hnsw — serialized neighbor-graph for fast ANN queries (rebuilt from DB on startup)
/// </summary>
public sealed class EmbeddedVectorStore : IVectorStore, IDisposable
{
    ...
    private readonly ConcurrentDictionary<string, float[]> _vectors = new();          // id → embedding
    private readonly ConcurrentDictionary<string, List<string>[]> _graph = new();     // id → layers → neighbor ids
    private readonly ConcurrentDictionary<string, ChunkMeta> _metaCache = new();      // id → lightweight meta
